...
#	$model_no == 1 || next; 
	...
	foreach $filter_model (@filter_list)
	{
		...
		if ($model_name eq $fmodel_name || ($model_name =~ /^MULTICOM/ && $fmodel_name =~/^MULTICOM/) || ($model_name =~ /^RBO/ && $fmodel_name =~/^RBO/) || ($model_name =~ /^Pcons/ && $fmodel_name =~/^Pcons/) || ($model_name =~ /^FALCON/ && $fmodel_name =~/^FALCON/))
