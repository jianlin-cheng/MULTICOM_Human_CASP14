...
$tools_dir = "/home/jianliu/MULTICOM_Human_CASP14/tools";
$databases_dir = "/home/jianliu/MULTICOM_Human_CASP14/databases";
$scripts_dir = "/home/jianliu/MULTICOM_Human_CASP14/scripts";

$tm_score = "$tools_dir/TMscore_32";
$q_score = "$tools_dir/pairwiseQA/q_score";
$pdb2casp2 = "$scripts_dir/pdb2casp.pl";
#prosys dir
$prosys_dir = "$scripts_dir/prosys/";
#model dir
$modeller_dir = "$tools_dir/modeller-9.16/";
$domain_split_comb = 0; 
if(@ARGV<3)
{
...
my($dncon4_contactfile) = $ARGV[4]; # path,  NULL, dncon4 rr file
my($distmap_file) = $ARGV[5]; # path,  NULL, dist map file
...
if(!defined($dncon4_contactfile))
{
	$dncon4_contactfile='NULL';
}
else
{
	$dncon4_contactfile=abs_path($dncon4_contactfile);
}

if(!defined($distmap_file))
{
        $distmap_file='NULL';
}
else
{
        $distmap_file=abs_path($distmap_file);
}
...
else
{
 ...
 $tar = "http://www.predictioncenter.org/download_area/CASP14/server_predictions/$targetname.stage2.3D.srv.tar.gz";
}
...
print("$scripts_dir/filter_model_same_group_v2.pl $name.mlist $name.nlist  $tools_dir/tm_score2/TMscore > $same_group_log\n\n");
system("$scripts_dir/filter_model_same_group_v2.pl $name.mlist $name.nlist  $tools_dir/tm_score2/TMscore > $same_group_log");
...
my($wen_path_scwrl)="$tools_dir/scwrl4/Scwrl4";
...
if(-e "$dom_hhsearch_out/domain_info")
{
	...
	print("perl $scripts_dir/P1_get_domains_by_hhsearch15.pl $addr_fasta $dom_hhsearch_out $databases_dir/hhsearch1.5_db/hhsearch15db 40\n");
	system("perl $scripts_dir/P1_get_domains_by_hhsearch15.pl $addr_fasta $dom_hhsearch_out $databases_dir/hhsearch1.5_db/hhsearch15db 40");
...
if(-e "$disorder_out/$name.fasta.disorder")
{
	...
	print("sh $tools_dir/disorder_new/bin/predict_diso.sh $addr_fasta $disorder_out/$name.fasta.disorder\n");
	system("sh $tools_dir/disorder_new/bin/predict_diso.sh $addr_fasta $disorder_out/$name.fasta.disorder");
}
...
$final_ranking3 = $full_pred_dir."/eva_DeepRank3/DeepRank3_Cluster.txt";
$final_ranking4 = $full_pred_dir."/eva_DeepRank3/DeepRank3_SingleQA_lite.txt";
$final_ranking5 = $full_pred_dir."/eva_DeepRank3/DeepRank3_SingleQA.txt";

#check dncon4 rr file and dist map file
($full_dncon4_contactfile, $full_distmap_file) = &check_dncon4_and_dist_files($targetname, $addr_fasta, $full_pred_dir, $dncon4_contactfile, $distmap_file);

if(-e $final_ranking and -e $final_ranking3 and -e $final_ranking4 and -e $final_ranking5)
{
	print "Full_length evaluation ($final_ranking, $final_ranking2, $final_ranking3, $final_ranking4, $final_ranking5) has been finished!\n\n";
}
else
{
	print "Start to run full_length evaluation!\n\n";
	print("perl /home/jianliu/MULTICOM_Human_CASP14/scripts/Auto_human_server.pl $name $addr_fasta $full_pred_dir $full_dncon4_contactfile $full_distmap_file $dir_models &> $full_pred_dir/auto_human_server.log\n\n");
	system("perl /home/jianliu/MULTICOM_Human_CASP14/scripts/Auto_human_server.pl $name $addr_fasta $full_pred_dir $full_dncon4_contactfile $full_distmap_file $dir_models &> $full_pred_dir/auto_human_server.log");

	print("perl /home/jianliu/MULTICOM_Human_CASP14/scripts/sort_deep_qa_score.pl $full_pred_dir/eva/HumanQA_gdt_prediction.txt $full_pred_dir/eva/HumanQA_gdt_prediction_sort.txt\n\n");
	system("perl /home/jianliu/MULTICOM_Human_CASP14/scripts/sort_deep_qa_score.pl $full_pred_dir/eva/HumanQA_gdt_prediction.txt $full_pred_dir/eva/HumanQA_gdt_prediction_sort.txt");
...
#		print "$qlen - $prev_idx\n";
	...
	for ($i = 0; $i < @domain_start; $i++)
	{
		...
		($domain_dncon4_contactfile, $domain_distmap_file) = &check_dncon4_and_dist_files("domain${i}", $domain_fasta, $domain_pred_dir, "", "");
		####### (1) run full-length human structure prediction 
		...
		$domain_final_ranking3 = $domain_pred_dir."/eva_DeepRank3/DeepRank3_Cluster.txt";
		$domain_final_ranking4 = $domain_pred_dir."/eva_DeepRank3/DeepRank3_SingleQA_lite.txt";
		$domain_final_ranking5 = $domain_pred_dir."/eva_DeepRank3/DeepRank3_SingleQA.txt";
		if(-e $domain_final_ranking and -e $domain_final_ranking3 and -e $domain_final_ranking4 and -e domain_final_ranking5)
		{
			print "Domain_based evaluation ($domain_final_ranking, $domain_final_ranking2, $domain_final_ranking3, $domain_final_ranking4, $domain_final_ranking5) has been finished!\n\n";
		}
        else
        {
			print "Start to run domain evaluation!\n\n";
			print("perl $scripts_dir/Auto_human_server.pl domain${i} $domain_fasta $domain_pred_dir $domain_dncon4_contactfile $domain_distmap_file $domain_dir\n\n");
			system("perl $scripts_dir/Auto_human_server.pl domain${i} $domain_fasta $domain_pred_dir $domain_dncon4_contactfile $domain_distmap_file $domain_dir");
				
			print("perl $scripts_dir/sort_deep_qa_score.pl $domain_pred_dir/eva/HumanQA_gdt_prediction.txt $domain_pred_dir/eva/HumanQA_gdt_prediction_sort.txt\n\n");
			system("perl $scripts_dir/sort_deep_qa_score.pl $domain_pred_dir/eva/HumanQA_gdt_prediction.txt $domain_pred_dir/eva/HumanQA_gdt_prediction_sort.txt");
		}
	#### perform combination 
...
if ($domain_split_comb == 1)
{
	...
	for ($i = 0; $i < $domain_num; $i++)
	{
        ...
        $rank_file =  "$dir_output/dom${i}_TS/eva_DeepRank3/DeepRank3_Cluster.txt";
	...
	for ($i = 0; $i < $final_model_num; $i++)
	{
		...
		print("$prosys_dir/pir2ts_energy.pl $modeller_dir $atom_dir $comb_dir $pir_file $cm_model_num\n\n");
		system("$prosys_dir/pir2ts_energy.pl $modeller_dir $atom_dir $comb_dir $pir_file $cm_model_num");
		...
		if ( -f "$comb_dir/comb$idx.pdb")
		{
			...
			system("$tools_dir/scwrl4/Scwrl4 -i $model_file.org -o $comb_dir/casp$idx.scw >/dev/null");
			system("$scripts_dir/clash_check.pl $name.fasta $comb_dir/casp$idx.scw > $comb_dir/clash$idx.txt");
...
sub check_dncon4_and_dist_files
{
	my $targetname = shift;
	my $seq_file = shift;
	my $output_dir = shift;
	my $dncon4_contactfile = shift;
	my $distmap_file = shift;

	#check dncon4 rr file and dist map file
	$QA_files_folder = "$output_dir/QA_files";
	if(!-s $QA_files_folder)
	{
		`mkdir $QA_files_folder`;
	}

	print("\n###########Checking contact map and distance map###########\n");

	if(-e $distmap_file and -e $dncon4_contactfile)
	{
		`cp $distmap_file $QA_files_folder/$targetname.txt`;
		`cp $dncon4_contactfile $QA_files_folder/$targetname.rr`;
		$distmap_file = "$QA_files_folder/$targetname.txt";
		$dncon4_contactfile = "$QA_files_folder/$targetname.rr";
	}
	elsif(-e "$QA_files_folder/$targetname.txt" and -e "$QA_files_folder/$targetname.rr")
	{
		$distmap_file = "$QA_files_folder/$targetname.txt";
		$dncon4_contactfile = "$QA_files_folder/$targetname.rr";
	}
	...
	{
	    $dist_dir = "$QA_files_folder/deepdist";
	    -d "$dist_dir" || `mkdir $dist_dir`;
		$cmd = "sh /home/jianliu/MULTICOM_Human_CASP14/DeepRank3/tools/deepdist/predictors/ensemble/pred_deepdist1.sh $seq_file $dist_dir &> $QA_files_folder/run_dist.log";
		print("\ngenerating contact and distance map file: $cmd \n");
		$ren_return_val = system("$cmd");
		if ($ren_return_val)
		{
			print "ERROR! deepdist execution failed!";
			exit 0;
		}
		`cp $dist_dir/pred_map_ensem/rr/$targetname.rr $QA_files_folder/$targetname.rr`;
		`cp $dist_dir/pred_map_ensem/$targetname.txt $QA_files_folder/$targetname.txt`;
		$distmap_file = "$QA_files_folder/$targetname.txt";
		$dncon4_contactfile = "$QA_files_folder/$targetname.rr";
	}

	if(!-e $distmap_file or !-e $dncon4_contactfile)
	{
		print "ERROR! cannot find contact or distance map file: $dncon4_contactfile, $distmap_file!";
		exit 0;
	}

	return ($dncon4_contactfile, $distmap_file);
}
