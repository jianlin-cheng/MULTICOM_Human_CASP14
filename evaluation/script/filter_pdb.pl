...
while($counter < $length){
	...
	`perl pdb2seq.pl $path/$file_unzipped A > $path/$pdb_orig_seq_file`; #parse the sequence from the downloaded pdb files;   #need to uncommentted to make program run!!
	print "perl pdb2seq.pl $path/$file_unzipped A > $path/$pdb_orig_seq_file\n";
	...
	print "$clustalw_tool  -MATRIX=BLOSUM -TYPE=PROTEIN -INFILE=$align_path/$align_seq_file -OUTFILE=$align_out_file";
	`$clustalw_tool  -MATRIX=BLOSUM -TYPE=PROTEIN -INFILE=$align_path/$align_seq_file -OUTFILE=$align_out_file`;

	print "pdb original seq:".$pdb_orig_seq."\n";
	print "CASP_originl seq:".$casp_orig_seq."\n";
	...
	if($ratio > 0.2 || length($pdb_orig_seq) == 0){ # add by Jie
		print "\n\t\t Changing to chain B\n\n";
		my $pdb_orig_seq_file = "$target_ids[$counter]".'_pdb_orig_seq';
        	`perl pdb2seq.pl $path/$file_unzipped B > $path/$pdb_orig_seq_file`; #parse the sequence from the downloaded pdb files;

        	#my $casp_orig_seq_file = $casp_orig_seq_path.'/'.$target_ids[$counter].'_casp9.seq';
        	#my $casp_orig_seq;
        	#my $pdb_orig_seq;
        	#open(CASPSEQ, "<$casp_orig_seq_file");
        	#while(<CASPSEQ>){
...
CHGC:	} #end of changing chain;
	...
	while($counter_align_seq < $len_align_seq){
		my $cell_casp = substr($casp_align_seq, $counter_align_seq, 1);
		my $cell_pdb = substr($pdb_align_seq, $counter_align_seq, 1);
		if($cell_casp ne "-"){
			$counter_casp_seq++;
		}
		if($cell_pdb ne "-"){
